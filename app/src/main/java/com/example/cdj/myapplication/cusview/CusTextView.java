...
//        lineHeight = (int) (-mAscent + mPaint.descent()) + getPaddingTop() + getPaddingBottom();
//        lineHeight=bounds.bottom-bounds.top;
            lineHeight =mPaint.descent()-mPaint.ascent();
        maxLines=-1;
        lineList.clear();
        calculate();
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for(int i=0;i<lineList.size();i++)
        {
            if(i==0)
            {
                canvas.drawText(mText, lineList.get(i).startIndex, lineList.get(i).endIndex, paddingLeft, paddingTop+lineHeight-mPaint.descent(), mPaint);
            }
            else
            {
