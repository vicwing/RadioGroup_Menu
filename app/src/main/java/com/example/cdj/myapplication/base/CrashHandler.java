...
//		ex.printStackTrace();
		if (!handleException(ex) && mDefaultHandler != null) {
			...
			Logger.d("999999999999999999999999999999999999999999");
	...
	private boolean handleException(Throwable ex) {
		...
		collectCrashDeviceInfo(mContext); // 保存错误报告文件 //
		String crashFileName = saveCrashInfoToFile(ex); // 发送错误报告到服务器 //
	...
	private String saveCrashInfoToFile(Throwable ex) {
		...
		Logger.d("11111111111111111111111111111111");
		...
		Logger.d("2222222222222222222222222");
		try {
			...
			trace.flush();
			Logger.d("333333333333333333333333333333");
			trace.close();
			return fileName;
		} catch (Exception e) {
			Log.e(TAG, "an error occured while writing report file...", e);
			Logger.d("4444444444444444444444444444");
		}
		return null;
	}
